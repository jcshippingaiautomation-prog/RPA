// ============================================================
//  Inline SVG icon set (Lucide-style, single-color line icons)
//  ใช้ currentColor → ปรับสีตาม context ได้
//  วิธีใช้:
//    HTML:  <i data-icon="package"></i>   → ถูกแทนด้วย SVG ตอนโหลด
//    JS:    svgIcon("check")              → คืน string ของ SVG
// ============================================================
(function () {
  const P =
  ...
  // path/inner ของแต่ละ icon (viewBox 24x24)
  const ICONS = {
    // brand / nav
    bot:
  ...
  /** คืน SVG string ของ icon (ปรับ size ได้) */
  function svgIcon(name, size) {
    const inner = ICONS[name] || ICONS.circle;
    const s = size || 18;
    return (
      `<svg class="icon" width="${s}" height="${s}" viewBox="0 0 24 24" ${P} ` +
      `aria-hidden="true">${inner}</svg>`
    );
  }

  /** แทน <i data-icon="name" [data-size="20"]></i> ทั้งหน้าด้วย SVG */
  function renderIcons(root) {
    (root || document).querySelectorAll("[data-icon]").forEach((el) => {
      const name = el.getAttribute("data-icon");
      const size = el.getAttribute("data-size");
      el.innerHTML = svgIcon(name, size ? Number(size) : undefined);
