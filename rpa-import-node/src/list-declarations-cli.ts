...
try {
  ...
  await sleep(6000);
  // ⚠ DCTK โหลดตารางด้วย ajax — บางวันช้ากว่า 6 วินาที
  //   ถ้าอ่านเร็วเกินจะได้ 0 แถว แล้วสรุปผิดว่า "ไม่มีใบขน/ตารางเปลี่ยนโครง"
  //   → รอจนมีแถวจริง (สูงสุด ~60s) ก่อนไปต่อ
  for (let w = 0; w < 30; w++) {
    const n = await page.locator("#grid tbody tr").count().catch(() => 0);
    if (n > 0) { log(`   ✓ ตารางพร้อม (${n} แถวแรก)`); break; }
    if (w === 29) log("   ⚠ รอ 60s แล้วตารางยังว่าง — อ่านต่อเท่าที่ได้");
    await sleep(2000);
  }
